...
export class AuthService {
  ...
  constructor(private httpClient:HttpClient,private localStorage:LocalStorageService) {
   ...
   {
     return this.httpClient.post('http://localhost:8484/api/auth/signup',signupRequest,{responseType:'text'});
   }

   login(loginRequest:LoginRequest):Observable<boolean>
   {
     return this.httpClient.post<LoginResponse>('http://localhost:8484/api/auth/login',loginRequest).pipe(map(data=>{
